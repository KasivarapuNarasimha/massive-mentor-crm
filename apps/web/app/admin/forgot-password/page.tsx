        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8">
          {sent ? (
            <div className="space-y-4 text-sm text-zinc-300">
              ...
              <div className="rounded-xl border border-violet-800/40 bg-violet-950/20 p-3 text-xs text-violet-100/90 space-y-2">
                ...
                <p className="text-zinc-400">
                  ...
                  editing SMTP_*, restart the API — <code className="text-zinc-300">tsx watch</code> does not
                  reload <code className="text-zinc-300">.env</code>.
                </p>
              </div>
            ...
            <form onSubmit={onSubmit} className="space-y-5">
              <div>
                <label htmlFor="afp-email" className="block text-sm text-zinc-300 mb-2">
                  Staff email
                </label>
                <input
                  id="afp-email"
              ...
              <button
                type="submit"
                disabled={busy}
                className="w-full min-h-12 bg-violet-500 text-zinc-950 font-semibold rounded-xl disabled:opacity-50"
              >
                {busy ? "Sending…" : "Send reset link"}
              </button>
              <p className="text-center text-sm text-zinc-500">
                <Link href="/admin/login" className="hover:text-zinc-300 underline">
                  Back to sign in
                </Link>
              </p>
            </form>
