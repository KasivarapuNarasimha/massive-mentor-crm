    ...
    <div className="w-full max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8 overflow-x-hidden pb-24 md:pb-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Personalized weekly action plan based on your business profile</p>
        </div>

        <button
          onClick={handleGenerate}
          disabled={isGenerating || isLoading}
          className="px-6 py-2.5 bg-white text-zinc-950 rounded-xl font-medium hover:bg-zinc-200 focus-ring button-active transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
      ...
      {isLoading ? (
        <div className="animate-pulse space-y-6">
          ...
          {/* Week cards skeleton */}
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-44 bg-zinc-900 border border-zinc-800 rounded-2xl" />
          ))}
        </div>
      ) : roadmap ? (
        <div className="space-y-8">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="text-xs px-3 py-1 bg-zinc-800 text-zinc-400 rounded-full w-fit">
                4 Weeks • 30 Days
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            {roadmap.days?.weeks?.map((week: Week, index: number) => (
              <div key={index} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-7">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-xs px-2.5 py-1 bg-zinc-800 text-zinc-400 rounded">
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 mt-4">
                  {week.tasks.map((task: string, taskIndex: number) => (
                    <li key={taskIndex} className="flex gap-3 text-sm leading-relaxed text-zinc-200">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 sm:p-12 text-center">
          ...
          <button
            ...
            className="px-8 py-3 bg-white text-zinc-950 rounded-xl font-medium hover:bg-zinc-200 focus-ring button-active transition-colors disabled:opacity-60"
          >
