    ...
    <div className="w-full max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 overflow-x-hidden pb-24 md:pb-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Strategic analysis powered by AI</p>
        </div>

        <button
          onClick={handleGenerate}
          disabled={isGenerating || isLoading}
          className="px-6 py-2.5 bg-white text-zinc-950 rounded-xl font-medium hover:bg-zinc-200 focus-ring button-active transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
        >
      ...
      {isLoading ? (
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-64 bg-zinc-900 border border-zinc-800 rounded-2xl" />
            ))}
          </div>
        </div>
      ) : swot ? (
        <div className="space-y-8">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8">
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm font-medium text-zinc-400 tracking-widest">EXECUTIVE SUMMARY</div>
              <div className="text-xs text-zinc-500">
                Generated on {formatDate(swot.createdAt)} • {swot.aiModel}
              </div>
            </div>
            <p className="text-lg leading-relaxed text-zinc-200">{swot.summary}</p>
          </div>
                ...
                <div key={key} className={`${colors.bg} border ${colors.border} rounded-2xl p-6 sm:p-7`}>
                  ...
                  <ul className="space-y-3 mt-5">
                    {items.length > 0 ? (
                      ...
                      <li className="text-sm text-zinc-500 italic">No items generated</li>
                    )}
                  </ul>
                </div>
              );
