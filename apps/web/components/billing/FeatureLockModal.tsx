  ...
  return (
    ...
    >
      <button
        ...
        onClick={onClose}
      />
      <div className="relative w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl border border-zinc-700/80 bg-gradient-to-b from-zinc-900 to-zinc-950 shadow-2xl shadow-black/50 p-6 sm:p-8 animate-in slide-in-from-bottom duration-200">
        ...
        <p className="mt-3 text-center text-sm leading-relaxed text-zinc-400">
          ...
          {featureLabel ? (
            <>
              ...
              <span className="text-zinc-200">{featureLabel}</span>.
            </>
          ) : null}
        </p>
        <div className="mt-7 flex flex-col-reverse sm:flex-row gap-2.5">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 min-h-11 rounded-xl border border-zinc-700 bg-zinc-900/80 text-sm font-medium text-zinc-300 hover:bg-zinc-800 transition-colors"
          >
            Maybe Later
          </button>
          <Link
            ...
            onClick={onClose}
            className="flex-1 min-h-11 inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-violet-600 to-sky-600 text-sm font-semibold text-white shadow-lg shadow-violet-900/30 hover:from-violet-500 hover:to-sky-500 transition-all"
          >
            Upgrade Now
          </Link>
        </div>
