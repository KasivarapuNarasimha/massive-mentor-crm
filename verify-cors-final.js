...
(async () => {
  const email = 'cors-final-' + Date.now() + '@ex.com';
  console.log('Testing OPTIONS from 3001:');
  let o = await doReq('/api/auth/register', 'OPTIONS', { 'Origin': 'http://localhost:3001', 'Access-Control-Request-Method': 'POST' });
  console.log('OPTIONS 3001 status:', o.status);
  console.log('ACA-Origin:', o.headers['access-control-allow-origin']);
  console.log('ACA-Creds:', o.headers['access-control-allow-credentials']);
  console.log('\nFull POST register from 3001:');
  let r = await doReq('/api/auth/register', 'POST', { 'Origin': 'http://localhost:3001' }, { email, password: 'Test123!', name: 'FinalTest' });
  ...
  console.log('Login body:', r.body.substring(0, 120));
  console.log('\nOPTIONS from 3000:');
  o = await doReq('/api/auth/register', 'OPTIONS', { 'Origin': 'http://localhost:3000', 'Access-Control-Request-Method': 'POST' });
  console.log('OPTIONS 3000 status:', o.status, 'origin:', o.headers['access-control-allow-origin']);
